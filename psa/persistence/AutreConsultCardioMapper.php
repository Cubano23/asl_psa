<?php 
	...
	class AutreConsultCardioMapper extends SelfManagedMapper{
		...
		function getdernierExamsRCVA($id){
			$query = "SELECT max(dChol) as dChol, max(dHDL) as dHDL, max(dLDL) as dLDL, max(dtriglycerides) as dtriglycerides, ".
					 ...
					 "max(darret) as darret, max(spirometrie_date) as spirometrie_date, max(spirometrie_status) as spirometrie_status, max(spirometrie_rapport_VEMS_CVF) as spirometrie_rapport_VEMS_CVF, ".
					 "max(spirometrie_CVF) as spirometrie_CVF, max(spirometrie_VEMS) as spirometrie_VEMS, max(spirometrie_DEP) as spirometrie_DEP,".
		...
		function getExpiredExams($cabinet,$period=0){
			$query =  "select ".$this->getTableName().".id, date, poids, max(dpoids) as dpoids, spirometrie, max(spirometrie_date) as spirometrie_date,  TaSys, TaDia, TA_mode, ".
		...
		function getSpirometrieRCVA($id, $spirometrie_date)
		{
			$query =  "select spirometrie_rapport_VEMS_CVF ".
					  "from ".$this->getTableName()." where ".
					  "id = '$id'  AND spirometrie_date='$spirometrie_date' ".
					  "GROUP BY id";
			$result = $this->findAnyRows($query);
		...
		{
			$query =  "select spirometrie_date, spirometrie_rapport_VEMS_CVF ".
					  "from ".$this->getTableName()." where ".
		...
		function getPremierAutreExam($id){
			$query =  "select  antecedants, traitement, dosage, hypertenseur3, automesure, diuretique, HVG, activite, ".
					  "tabac, darret, spirometrie_date, spirometrie_status, spirometrie_CVF, spirometrie_VEMS, spirometrie_DEP, spirometrie_rapport_VEMS_CVF from ".
					  $this->getTableName()." where id = '$id' ORDER BY date ";
		...
		function Liste_consult($id){
			$query =  "select date, date_format(date, '%d/%m/%Y') as date_affiche ".
