...
$fichier="./log/chambery1/".$list[0]["filename"];
	...
	//préparation des feuilles de compte-rendu et écriture des lignes de titre
	...
	//Liste des mots-clés
	$mots=array("Microalbuminurie des 24 h"=>"albu",
				"CHOLESTEROL TOTAL.."=>"Chol",
				"HDL Cholesterol"=>"HDL",
				"CHOLESTEROL.H.D.L."=>"HDL",
				"Cholestérolémie HDL par précipitation"=>"HDL",
				"LDL CHOLESTEROL"=>"LDL",
				"Cholestérolémie LDL par précipitation"=>"LDL",
				"LDL Cholesterol"=>"LDL",
				"Créatinine"=>"creat",
				"Créatininémie."=>"creat",
				"CREATININE"=>"creat",
				...
				"GLYCEMIE à jeun"=>"glycemie",
				"HEMOGLOBINE GLYCOSYLEE HB"=>"HBA1c",
	...
	while($ligne=fgets($fp)){
		...
		if((count($tab_ligne)>2)&&(isset($mots[$tab_ligne[0]]))){//On est sur un mot clé => on cherche à intégrer
			...
			if(isset($unites[$mots[$tab_ligne[0]]])&&(in_array($unite,$unites[$mots[$tab_ligne[0]]]))){//On est sur un examen dans la mauvaise unité=>à convertir
			...
			if($date>"2011-01-01"){
				foreach($mots as $exam=>$code){
					if(strpos($ligne, $exam)!==false){//Le mot clé est indiqué dans la ligne
						if($code=="albu"){//Dans le cas de l'albu, il faut regarder les lignes suivantes pour avoir le résultat
						...
						if($code=="HBA1c"){//Dans le cas de l'HBA1c, il faut regarder les lignes suivantes pour avoir le résultat
