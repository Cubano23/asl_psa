<?php
function integration_mont_de_marsan($fichier, $fichier_name){

	$upfile="./log/marsan/".$fichier_name;
...
if ($archive->extract(PCLZIP_OPT_PATH, "./log/marsan/") == 0) {
...
$fichier="./log/marsan/".$list[0]["filename"];
	...
	#$contrCreat = controleValeur::controleCreatinine();
	...
	while($ligne=fgets($fp)){
		...
		if($mots[$colone1]=='creat' && $tab_ligne[2]>100){
		 	$controleCreat=FALSE;
		 }
		 else{
		 	$controleCreat=TRUE;
		...
		if( ($record) && ($fauxamiscond) && ($controleCreat) ) {
			...
			if( (count($tab_ligne)>4 ) && ( isset($mots[$colone1])) ){//On est sur un mot cle => on cherche a integrer
						...
						if($date>"2008-01-01"){
							...
							if(mysql_num_rows($res2)==0){//La donnee n'est pas presente dans asalee
								if($valeur==""){
									...
									if(($mots[$exam]!="ECG")&&(!is_numeric($valeur))||($unite==$minimol)){
										$lko++;
										$worksheet_ko->write("A$lko", "$id");
										$worksheet_ko->write_string("B$lko", "$numero");
										$worksheet_ko->write("C$lko", "$date");
