<?php
//antoine
function integration_saint_julien($fichier_ex, $fichier_pat){
	...
	$zones=array("nom", "prenom", "ville", "", "", "dnaiss", "", "", "", 
				 "", "tel", "", "", "id", "", "", "", "", "");
			...
			if(($id!="")&&($id>0)){
				...
				if(mysql_num_rows($res)==0){//Le même n° de patient n'a pas été trouvé
					$req2="INSERT INTO pat_mediclic SET numero='$id', ".
						  ...
						  "ville='".addslashes(stripslashes($ville))."', ".
						  "dnaiss='$dnaiss', tel='$tel', cabinet='".$_SESSION["cabinet"]."'";
						   // echo $req2;
	...
	$mots=array("HbA1c"=>"HBA1c",//ok
				"Hémoglobine glycosylée"=>"HBA1c",//ok
				"Hemoglobine Glycosylée"=>"HBA1c",//ok
				"Hemoglogine Glycosylée"=>"HBA1c",//ok
				"HDL Cholesterol"=>"HDL",//ok
				"LDL Cholesterol"=>"LDL",//ok
				"Cholesterol Total"=>"Chol",
				"Cholesterol"=>"Chol",
				"Triglycerides"=>"triglycerides",//ok
	...
	while($ligne=fgets($fp)){
		...
		$val=$ligne[11];
			...
			$req="SELECT numero from pat_mediclic WHERE nom='".addslashes(stripslashes($nom))."' ".
				 ...
				 "and ville='".addslashes(stripslashes($ville))."' ".
				 "and tel='$tel' and dnaiss='$dnaiss' and cabinet='".$_SESSION["cabinet"]."'";
			$res=mysql_query($req) or die("erreur SQL:".mysql_error()."<br />$req");
