...
function integration_artemare_mediclick($fichier_ex, $fichier_pat){
	...
	$mots=array("HbA1c"=>"HBA1c",
				...
				"Proteinurie des 24h"=>"proteinurie",
				"Hematurie"=>"hematurie"				
				);
						...
						if($id!=""){//Le dossier est reconnu
							...
							$date_apres=date("Y-m-d", mktime(1, 1, 1, $dateexam[1]  , $dateexam[2]+15, $dateexam[0]));
								...
								if(mysql_num_rows($res2)==0){//La donnée n'est pas présente dans asalée
									...
									$worksheet_ok->write("B$lok", $numero."M");
									...
									$worksheet_bis->write("B$lbis", $numero."M");
							...
							$worksheet_ko->write_string("B$lko", $numero."M");
