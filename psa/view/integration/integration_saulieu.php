<?php
function integration_saulieu($fichier, $fichier_name){

	$upfile="./log/saulieu/".$fichier_name;
...
if ($archive->extract(PCLZIP_OPT_PATH, "./log/saulieu/") == 0) {
...
$fichier="./log/saulieu/".$list[0]["filename"];
	...
	$minimol=array("mmol/L", "mmol/l", "umol/L", "umol/l");
					
	$debut=1;
	$recherche=0;
	$date1=$date2="";
	$record=$fauxamiscond=FALSE;
	...
	while($ligne=fgets($fp)){
		...
		$colone1=slugify($tab_ligne[0]);
			// if($record){
			// 	#echo $colone1.'<br/>';
			// 	if($tab_ligne[6]==$minimol){
			// 		$controleMinimol=FALSE;
			// 	}
			// 	else{
			// 		$controleMinimol=FALSE;
			// 	}
					...
					if($id!=""){//Le dossier est reconnu
						if($date>"2008-01-01"){
							...
							if(mysql_num_rows($res2)==0){//La donnee n'est pas presente dans asalee
									...
									if(($mots[$exam]!="ECG")&&(!is_numeric($valeur))||(in_array($unite, $minimol)) ){
										$lko++;
										$worksheet_ko->write("A$lko", "$id");
										$worksheet_ko->write_string("B$lko", "$numero");
										$worksheet_ko->write("C$lko", "$date");
