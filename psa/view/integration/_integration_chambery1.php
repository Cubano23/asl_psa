...
$fichier="./log/chambery1/".$list[0]["filename"];
	...
	while($ligne=fgets($fp)){
		...
		if( ($record) && ($fauxamiscond) && ($controleCreat) && ($controleLDL)) {
			...
			if((count($tab_ligne)>2)&&(isset($mots[$colone1]))){//On est sur un mot clé => on cherche à intégrer
				$exam=$colone1;
				$valeur=$tab_ligne[2];
				$normal=$tab_ligne[3];
				$unite=$tab_ligne[6];

				if(isset($unites[$mots[$colone1]])&&(in_array($unite,$unites[$mots[$colone1]]))){//On est sur un examen dans la mauvaise unité=>à convertir
