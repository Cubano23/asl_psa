...
<pre style="align:left;"><?php //var_dump($liste); //exit(); ?></pre>
...
<!-- ZONE IDENTITAIRE | Header -->
		...
		<h2>Compétences et activités transverses Asalée<br>&nbsp;</h2>
		<h3 align="left" style="height:30px;"><a href="planning.php"><< Retour au planning</a></h3>
		
		<div style="text-align:left;">
			<a class="bouton7" id="affrech" style="text-align:left;cursor:pointer;text-align:center;padding-left:20px;padding-right:20px;">Fitrer par compétences et activités transverses Asalée</a>
			&nbsp;&nbsp;<a class="bouton7" style="text-align:left;cursor:pointer;text-align:center;padding-left:20px;padding-right:20px" href="modifier_competences.php">Modifier mes compétences et activités transverses Asalée</a>
		...
		<div id="rech" style="text-align:left;margin-top:10px;">
			...
			<div id="domaine" style="float:left;width:270px;margin-bottom:10px;">
				...
				<input type="checkbox" name="crit[]" value="apnee_sommeil" <?php if(in_array('apnees_sommeil',$_POST['crit'])){echo ' checked="checked"';} ?>> Apnée du sommeil<br />
				<input type="checkbox" name="crit[]" value="tabac_addict" <?php if(in_array('tabac_addict',$_POST['crit'])){echo ' checked="checked"';} ?>> Tabacologie, Addictologie<br />
                ...
                <input type="checkbox" name="crit[]" value="retinographie" <?php if(in_array('retinographie',$_POST['crit'])){echo ' checked="checked"';} ?>> Rétinographie<br />
			</div>
			...
			<div id="contrib" style="float:left;width:270px;margin-bottom:10px;">
				...
				<input type="checkbox" name="crit[]" value="transverses_compagnonnage" <?php if(in_array('transverses_compagnonnage',$_POST['crit'])){echo ' checked="checked"';} ?>> Référents "Compagnonnage"<br />
				<input type="checkbox" name="crit[]" value="transverses_reunion" <?php if(in_array('transverses_reunion',$_POST['crit'])){echo ' checked="checked"';} ?>> Référents  "Réunion sectorielle"<br />
				<input type="checkbox" name="crit[]" value="transverses_contact" <?php if(in_array('transverses_contact',$_POST['crit'])){echo ' checked="checked"';} ?>> Référents  "Contacts/Candidatures spontanées"<br />
				<input type="checkbox" name="crit[]" value="transverses_sevrage_tabac" <?php if(in_array('transverses_sevrage_tabac',$_POST['crit'])){echo ' checked="checked"';} ?>> Référents  "Sevrage tabagique"<br />
				<input type="checkbox" name="crit[]" value="transverses_apa" <?php if(in_array('transverses_apa',$_POST['crit'])){echo ' checked="checked"';} ?>> Référents "Activité Physique Adaptée (APA)"<br />
			</div>
			...
			<div id="logiciels2" style="float:left;width:270px;margin-bottom:10px;">
			    ...
			    <input type="checkbox" name="crit[]" value="mlm" <?php if(in_array('mlm',$_POST['crit'])){echo ' checked="checked"';} ?>> MLM<br />
		...
		<table width="98%" border="1" align="center" cellspacing="0">
			<tr>
				...
				<td> -->
				<td>
					<b>Activités transverses Asalée</b></td>
				<td>
				...
				$liste = UtilityControler::array_msort($liste,array('nom'=>SORT_ASC));

				foreach($liste as $comp){ 
					
					$inf = GetInfosByLogin($comp['login'], $status);
					echo "<!-- ";var_dump($inf); echo "-->";
					...
					foreach($cabs as $cab){
						...
						$cabinets .= utf8_encode($cabinet['nom_cab'])	.', ';
					}
					...
					$col3 = CompetencesControler::formatColonne(array("nutrition"=>$comp['nutrition'],"act_physique"=>$comp['act_physique'],"vigilance2"=>$comp['vigilance2'],"obesite"=>$comp['obesite'],"apnee_sommeil"=>$comp['apnee_sommeil'],"tabac_addict"=>$comp['tabac_addict'],"coord_geronto"=>$comp['coord_geronto'],"retinographie"=>$comp['retinographie']));
					...
					$col5_transverses = CompetencesControler::formatColonne(array(
						"transverses_compagnonnage"=>$comp['transverses_compagnonnage'],
						"transverses_reunion"=>$comp['transverses_reunion'],
						"transverses_contact"=>$comp['transverses_contact'],
						"transverses_sevrage_tabac"=>$comp['transverses_sevrage_tabac'],
						"transverses_apa"=>$comp['transverses_apa'],
					));
					$col6 = CompetencesControler::formatColonne(array("utilisation_portail_psa"=>$comp['utilisation_portail_psa'],"integration_donnees"=>$comp['integration_donnees'],"informatique"=>$comp['informatique'],"bureautique"=>$comp['bureautique'],"communication"=>$comp['communication']));
					$col7 = CompetencesControler::formatColonne(array("amalpro"=>$comp['amalpro'],"axisante4"=>$comp['axisante4'],"axisante5"=>$comp['axisante5'],"clinidoc"=>$comp['clinidoc'],"crossway"=>$comp['crossway'],"dbmed"=>$comp['dbmed'],"docware"=>$comp['docware'],"easyprat"=>$comp['easyprat'],"eomed"=>$comp['eomed'],"hellodoc"=>$comp['hellodoc'],"hellodoc5_55"=>$comp['hellodoc5_55'],"hellodoc5_6"=>$comp['hellodoc5_6'],"hypermed"=>$comp['hypermed'],"ict"=>$comp['ict'],"maldis"=>$comp['maldis'],"medi"=>$comp['medi'],"medicawin"=>$comp['medicawin'],"mediclick"=>$comp['mediclick'],"mediclick5"=>$comp['mediclick5'],"medimust"=>$comp['medimust'],"medistory"=>$comp['medistory'],"mediwin"=>$comp['mediwin'],"shaman"=>$comp['shaman'],"weda"=>$comp['weda'],"mlm"=>$comp['mlm']));
					...
					<tr>
						...
						<td align="center">
							<?php echo strtoupper($inf[0]['nom']).' '.$inf[0]['prenom'];?><br /><?php echo $inf[0]['telephone'];?>
							<br /><a href="mailto:<?php echo $inf[0]['email'];?>"><?php echo $inf[0]['email'];?></a><br /><?php echo $cabinets;?></td>
						<td>
							<?php echo $col1;?></td>
						<td>
							<?php echo $col2;?></td>
