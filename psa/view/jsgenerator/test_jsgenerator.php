...
class JSValidation {
		...
		?> 	
			...
			if(element != null && element.length == 1){
				...
				if(!validateDate(date)){	
					submitOk = 0
					alert("la date '<?php echo($fieldLabel) ?>' doit être au format 'jj/mm/aaaa', et l'année entre '1900' et '2100'.")
				}
	...
	function dateInRange($property,$fieldLabel){
		...
		?> 	
			...
			//alert ("<?php echo(($fieldLabel == 'Date colloscopie')||($fieldLabel == 'Date de mammographie'))?>");
            ...
            label3 = "<?php echo($fieldLabel == 'Date de naissance')?>";
				...
				else
				{
					if(label2=="1")
					{
					    ...
					    {
					        submitOk=0
					        alert("la date '<?php echo ($fieldLabel) ?>' doit être au format jj/mm/yyyy et être valide")
					    }
					}
					if(label3=="1")
					{
						...
						{
							submitOk = 0
							alert("la date '<?php echo($fieldLabel) ?>' doit être au format jj/mm/yyyy et inférieur à 1996")
						}
					}
					else
					{
	...
	function validateNumeroDossier($property,$fieldLabel){
		...
		?> 	
			...
			if(!validateNumeroDossier(field)){	
				submitOk = 0
				alert("le champ '<?php echo($fieldLabel)?>' n'est pas un numéro de dossier valide")
			}
		<?php
	}
	...
	function validateInteger($property,$fieldLabel){
		...
		?> 	
			...
			if(!validateInteger(field)){	
				submitOk = 0
				alert("le champ '<?php echo($fieldLabel)?>' doit être un entier")
			}
		<?php
	}
	...
	function validatePositiveInteger($property,$fieldLabel){
		...
		?> 	
			...
			if(!validatePositiveInteger(field)){	
				submitOk = 0
				alert("le champ '<?php echo($fieldLabel)?>' doit être un entier naturel")
			}
		<?php
	}
	...
	function validateNumeric($property,$fieldLabel){
		...
		?> 	
			...
			if(!validateNumeric(field)){	
				submitOk = 0
				alert("le champ '<?php echo($fieldLabel)?>' doit être numérique")
			}
		<?php
	}
	...
	function validatePositiveNumeric($property,$fieldLabel){
		...
		?> 	
			...
			if(!validatePositiveNumeric(field)){	
				submitOk = 0
				alert("le champ '<?php echo($fieldLabel)?>' doit être numérique et positif")
			}
		<?php
	}
	...
	function validateEmail($property,$fieldLabel){
		...
		?> 	
			...
			if(!validateEmail(field)){	
				submitOk = 0
				alert("le champ '<?php echo($fieldLabel)?>' doit être une adresse email valide")
			}
		<?php
	}
	...
	function validateSexe($property,$fieldLabel){
		...
		?> 	
			...
			if(!validateSexe(field)){	
				submitOk = 0
				alert("le champ '<?php echo($fieldLabel)?>' doit être 'Féminin' ou 'Masculin'")
			}
		<?php
	}
	...
	function validateAge($property,$fieldLabel){
		...
		?> 	
			...
			if(!validateAge(field)){	
				submitOk = 0
				alert("le champ '<?php echo($fieldLabel)?>' doit être un âge valide")
			}
		<?php
	}
	...
	function validateMois($property,$fieldLabel){
		...
		?> 	
			...
			if(!validateMois(field)){	
				submitOk = 0
				alert("le champ '<?php echo($fieldLabel)?>' doit être un numéro de mois valide")
			}
		<?php
	}
