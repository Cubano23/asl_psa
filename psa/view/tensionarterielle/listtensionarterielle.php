...
<CAPTION>Liste des <?php echo(count($tensionArterielleMoyenneList)); ?> suivis trouv&eacute;s</CAPTION>
<tr>
  <th>Début</th>
  <th>Durée</th>
  <th>Matin</th>
  ...
  <?php for($i=0;$i<count($tensionArterielleMoyenneList);$i++){ 
	...
	$req="select date_debut, date_add(date_debut, interval (nombre_jours-1) day) as date_fin, ".
         "group_id from tension_arterielle_moyenne where id = '".$dossier->id."' and date_debut = '$date_deb'";
         //echo $req;
         ...
         $ligne=mysql_fetch_assoc($res);
				...
				buildLink("","détail","$path/controler/ActionControler.php",$param->controler,ACTION_FIND, "", $additionalParams); 
			?>
</td>
		<td align = "center"><?php 
				...
				$additionalParams = array("Dossier:dossier:id"=>$tmp->id,
						...
						"tensionArterielleManagement:tensionArterielleManagement:dateFin"=>$ligne['date_fin'],
						"tensionArterielleManagement:tensionArterielleManagement:nombreJours"=>$tmp->nombre_jours,
						"tensionArterielleManagement:tensionArterielleManagement:group_id"=>$ligne["group_id"]);
				buildLink("onclick='return confirm(\"Etes-vous sûr de vouloir supprimer cette automesure ?\")'","Supprimer","$path/controler/ActionControler.php",$param->controler,ACTION_DELETE, "", $additionalParams); 
			?>
</td>
	</tr>
