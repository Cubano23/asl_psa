...
<?php global $outDateReference ?>
  ...
  <?php
	...
	$TroubleCognitifMapper = new TroubleCognitifMapper(NULL);

  	for($i=0;$i<count($rowsList);$i++){
		$dossier = $dossierMapper->doLoadObject($rowsList[$i]);
		$dossier = $dossier->afterDeserialisation($account);
		$TroubleCognitif = $TroubleCognitifMapper->doLoadObject($rowsList[$i]);
		$TroubleCognitif = $TroubleCognitif->afterDeserialisation($account);

	 	$dateRef = $TroubleCognitif->isOutdated($outDateReference->period);
		if($dateRef != false){
			$nb_dossier++;
		}
  }
	...
	$TroubleCognitifMapper = new TroubleCognitifMapper(NULL);
  	...
  	for($i=0;$i<count($rowsList);$i++){
		...
		if($dateRef != false){
			...
			require("listtroublecognitifalerterow.php");
