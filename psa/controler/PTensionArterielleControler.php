		...
		function start() {
			...
			switch($param->action){
				...
				case ACTION_LIST:
					...
					if($result == false){
						if($tensionArterielleMoyenneMapper->lastError == BAD_MATCH) forward($this->mappingTable["URL_AFTER_LIST"],"Aucune automesure antérieure de tension artérielle");
				...
				case ACTION_FIND:
					...
					$result = $tensionArterielleMoyenneMapper->findObjects($tensionArterielleMoyenne);
					if($result == false){
						if($tensionArterielleMoyenneMapper->lastError == BAD_MATCH) forward($this->mappingTable["URL_MANAGE"],"Pas d'enregistrements trouvés");
						else forward(URL_CONTROLER_PERSISTENCE_ERROR,"Find objects caused an error");				
					}
