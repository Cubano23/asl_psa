...
global $date_start; $date_start = $current_year.'-'.$current_month.'-01';
...
$diff_start = (7 - date_diff2($first_lundi_final, mktime(0, 0, 0, $current_month, 1, $current_year)) + 1);
  ...
  $total_temps = 0;
          ...
          foreach ($saisieInfirmiere as $key => $value) 
          {
            ...
            if(($value['date'] >= $date_start) && ($value['date'] <= $date_end))
            {
              ...
              $examsDerogatoire['spiro'] += ((($value['spirometre_seul'] != NULL) ? $value['spirometre_seul'] : 0) + (($value['spirometre'] != NULL) ? $value['spirometre'] : 0));
              $examsDerogatoire['cogn'] += $value['t_cognitif'];
              $examsDerogatoire['ecg'] += ((($value['ecg_seul'] != NULL) ? $value['ecg_seul'] : 0) + (($value['ecg'] != NULL) ? $value['ecg'] : 0));
...
$req = "SELECT count(*) FROM dossier as d INNER JOIN account as a ON d.cabinet=a.cabinet WHERE a.infirmiere!='' and a.region!='' AND d.actif='oui' AND (a.nom_cab like '%16%' OR a.nom_cab like '%17%' OR a.nom_cab like '%79%' OR a.nom_cab like '%86%')";
...
$req = "SELECT cabinet, d.id, date FROM evaluation_infirmier as e INNER JOIN dossier as d ON e.id=d.id WHERE actif='oui' AND d.cabinet IN (SELECT cabinet FROM account WHERE (nom_cab like '%16%' OR nom_cab like '%17%' OR nom_cab like '%79%' OR nom_cab like '%86%')) ORDER BY cabinet, id, date";
...
$req = "SELECT cabinet, d.id, date_exam, resultat1 FROM liste_exam INNER JOIN dossier as d ON d.id=liste_exam.id INNER JOIN cardio_vasculaire_depart AS c ON c.id=d.id WHERE actif='oui' and type_exam='systole' and date_exam > '2009-01-01' AND date_exam < '2014-04-30' and d.cabinet IN (SELECT cabinet FROM account WHERE (nom_cab like '%16%' OR nom_cab like '%17%' OR nom_cab like '%79%' OR nom_cab like '%86%')) GROUP BY cabinet, d.id, date_exam";
...
while(list($cabinet, $id, $date, $TaSys)=mysql_fetch_row($res)){
  ...
  //}
  //echo '<br>@@'.$id.' - '.$date.' : '.$TaSys.' / '.$TaDia;
}
// echo '<hr />';
// var_dump($liste_tension);
// echo '<hr />';
...
//      "and cabinet='".$current_cabinet."'".
//      "GROUP BY cabinet, dossier.id, date_exam ";
$req = "SELECT cabinet, d.id, date_exam, resultat1 FROM suivi_diabete, liste_exam INNER JOIN dossier AS d ON d.id=liste_exam.id WHERE actif='oui' AND dossier_id=d.id and date_exam > '2009-01-01' AND date_exam < '2014-04-20' and type_exam='systole' and d.cabinet IN (SELECT cabinet FROM account WHERE (nom_cab like '%16%' OR nom_cab like '%17%' OR nom_cab like '%79%' OR nom_cab like '%86%')) GROUP BY cabinet, d.id, date_exam";
...
foreach($liste_tension as $id => $tab)
{
  if(isset($consult[$id]))
  {
    ...
    if($valeur1=="")
    {
      ...
      $dossierspastension=$dossierspastension+1;
    }elseif($valeur2!=""){
      if(($valeurs["TaSys"]<140)&&($valeurs["TaDia"]<90))
      {//Dossier <140/90
        ...
        $dossiersinf140[1]=$dossiersinf140[1]+1;
      }else{
        ...
        $dossierssup140[1]=$dossierssup140[1]+1;
      ...
      {//Dossier <140/90
        ...
        $dossiersinf140[2]=$dossiersinf140[2]+1;
      }else{
        ...
        $dossierssup140[2]=$dossierssup140[2]+1;
      ...
      {//Dossier <140/90
        ...
        $dossiersinf140[3]=$dossiersinf140[3]+1;
      }else{
        ...
        $dossierssup140[3]=$dossierssup140[3]+1;
      ...
      {//Dossier <140/90
        ...
        $dossiersinf140[4]=$dossiersinf140[4]+1;
      }else{
        ...
        $dossierssup140[4]=$dossierssup140[4]+1;
...
$req="SELECT dossier.cabinet, count(*), nom_cab, region ".
     ...
     "AND actif='oui' ".
     "and dossier.cabinet=account.cabinet ".
     "AND account.cabinet IN (SELECT cabinet FROM account WHERE (nom_cab like '%16%' OR nom_cab like '%17%' OR nom_cab like '%79%' OR nom_cab like '%86%')) ".     // ICI MODIF 11/02/14
     "GROUP BY nom_cab ".
     "ORDER BY nom_cab, numero ";
...
$req="SELECT cabinet, dossier.id, date ".
     ...
     "WHERE actif='oui' ".
     "AND evaluation_infirmier.id=dossier.id ".
     "AND cabinet IN (SELECT cabinet FROM account WHERE (nom_cab like '%16%' OR nom_cab like '%17%' OR nom_cab like '%79%' OR nom_cab like '%86%')) ".
     "ORDER BY cabinet, id, date ";
...
$dossierssup140['pierre'][1] = $dossierssup140['pierre'][2] = $dossierssup140['pierre'][3] = $dossierssup140['pierre'][4] = 0;
$dossiersinf140['pierre'][1] = $dossiersinf140['pierre'][2] = $dossiersinf140['pierre'][3] = $dossiersinf140['pierre'][4] = 0;
$change['pierre'][1] = $change['pierre'][2] = $change['pierre'][3] = $change['pierre'][4] = 0;
$req="SELECT cabinet FROM account WHERE region<>'' AND infirmiere<>'' AND(nom_cab like '%16%' OR nom_cab like '%17%' OR nom_cab like '%79%' OR nom_cab like '%86%') ORDER BY cabinet";
$res=mysql_query($req) or die("erreur SQL:".mysql_error()."<br>$req");
while($tab = mysql_fetch_array($res))
{
  if(is_array($dossierssup140[$tab[0]])) 
  {
    //echo '<br />'.$tab[0].' : '.$dossierssup140[$tab[0]];
    // echo '<hr /><pre>';
    // var_dump($dossierssup140[$tab[0]]);
    // echo '</pre>';
    for($i = 1; $i<5; $i++)
    {
      $dossierssup140['pierre'][$i] += $dossierssup140[$tab[0]][$i];
      $dossiersinf140['pierre'][$i] += $dossiersinf140[$tab[0]][$i];
      $change['pierre'][$i] += $change[$tab[0]][$i];
    }
  }
}
echo '<hr />@@@@@@@@@@@';
echo '<hr /><pre>';
var_dump($dossierssup140['pierre']);
echo '</pre><hr /><pre>';
var_dump($dossiersinf140['pierre']);
echo '</pre><hr /><pre>';
var_dump($change['pierre']);
echo '</pre>';
...
//      "GROUP BY nom_cab ".
//      "ORDER BY nom_cab, numero ";
$req1 = "SELECT a.cabinet, nom_cab FROM dossier, account as a WHERE infirmiere!='' and region!='' AND dossier.cabinet=a.cabinet AND actif='oui' AND (a.nom_cab like '%16%' OR a.nom_cab like '%17%' OR a.nom_cab like '%79%' OR a.nom_cab like '%86%') GROUP BY nom_cab ORDER BY nom_cab, numero";
...
$req1 = "SELECT cabinet, dossier.id, date_exam, resultat1, evaluation_infirmier.date as date_consult, DATEDIFF(date_exam, evaluation_infirmier.date) as deltaj FROM dossier, liste_exam, evaluation_infirmier, suivi_diabete WHERE liste_exam.id=dossier.id AND evaluation_infirmier.id=dossier.id  and dossier.cabinet IN (SELECT cabinet FROM account WHERE (nom_cab like '%16%' OR nom_cab like '%17%' OR nom_cab like '%79%' OR nom_cab like '%86%')) and type_exam='HBA1c' and suivi_diabete.dossier_id=dossier.id and liste_exam.date_exam > '2009-01-01' AND liste_exam.date_exam < '2014-04-20' ORDER BY cabinet, dossier.id, date_consult, date_exam";
...
//      "GROUP BY nom_cab ".
//      "ORDER BY nom_cab, numero ";
$req2 = "SELECT a.cabinet, nom_cab FROM dossier, account as a WHERE infirmiere!='' and region!='' AND dossier.cabinet=a.cabinet AND actif='oui' AND (a.nom_cab like '%16%' OR a.nom_cab like '%17%' OR a.nom_cab like '%79%' OR a.nom_cab like '%86%') GROUP BY nom_cab ORDER BY nom_cab, numero";
...
//     "ORDER BY cabinet, dossier.id, date_consult, date_exam";
$req2 = "SELECT cabinet, dossier.id, date_exam, resultat1, evaluation_infirmier.date as date_consult, DATEDIFF(date_exam, evaluation_infirmier.date) as deltaj FROM dossier, liste_exam, evaluation_infirmier, suivi_diabete WHERE liste_exam.id=dossier.id AND evaluation_infirmier.id=dossier.id  and dossier.cabinet IN (SELECT cabinet FROM account WHERE (nom_cab like '%16%' OR nom_cab like '%17%' OR nom_cab like '%79%' OR nom_cab like '%86%')) and type_exam='HBA1c' and suivi_diabete.dossier_id=dossier.id and liste_exam.date_exam > '2009-01-01' AND liste_exam.date_exam < '2014-04-20' ORDER BY cabinet, dossier.id, date_consult, date_exam";
...
//      "GROUP BY nom_cab ".
//      "ORDER BY nom_cab, numero ";
$req3 = "SELECT a.cabinet, nom_cab FROM dossier, account as a WHERE infirmiere!='' and region!='' AND dossier.cabinet=a.cabinet AND actif='oui' AND (a.nom_cab like '%16%' OR a.nom_cab like '%17%' OR a.nom_cab like '%79%' OR a.nom_cab like '%86%') GROUP BY nom_cab ORDER BY nom_cab, numero";
...
//     "and suivi_diabete.dossier_id=liste_exam.id ".
//     "ORDER BY cabinet, dossier.id, date_consult, date_exam";
$req3 = "SELECT cabinet, dossier.id, date_exam, resultat1, evaluation_infirmier.date as date_consult, DATEDIFF(date_exam, evaluation_infirmier.date) as deltaj FROM dossier, liste_exam, evaluation_infirmier, suivi_diabete WHERE liste_exam.id=dossier.id AND evaluation_infirmier.id=dossier.id  and dossier.cabinet IN (SELECT cabinet FROM account WHERE (nom_cab like '%16%' OR nom_cab like '%17%' OR nom_cab like '%79%' OR nom_cab like '%86%')) and type_exam='HBA1c' and suivi_diabete.dossier_id=dossier.id and liste_exam.date_exam > '2009-01-01' AND liste_exam.date_exam < '2014-04-20' ORDER BY cabinet, dossier.id, date_consult, date_exam";
...
//      "GROUP BY nom_cab ".
//      "ORDER BY nom_cab, numero ";
$req4 = "SELECT a.cabinet, nom_cab FROM dossier, account as a WHERE infirmiere!='' and region!='' AND dossier.cabinet=a.cabinet AND actif='oui' AND (a.nom_cab like '%16%' OR a.nom_cab like '%17%' OR a.nom_cab like '%79%' OR a.nom_cab like '%86%') GROUP BY nom_cab ORDER BY nom_cab, numero";
...
//     "and suivi_diabete.dossier_id=liste_exam.id ".
//     "ORDER BY cabinet, dossier.id, date_consult, date_exam";
$req4 = "SELECT cabinet, dossier.id, date_exam, resultat1, evaluation_infirmier.date as date_consult, DATEDIFF(date_exam, evaluation_infirmier.date) as deltaj FROM dossier, liste_exam, evaluation_infirmier, suivi_diabete WHERE liste_exam.id=dossier.id AND evaluation_infirmier.id=dossier.id  and dossier.cabinet IN (SELECT cabinet FROM account WHERE (nom_cab like '%16%' OR nom_cab like '%17%' OR nom_cab like '%79%' OR nom_cab like '%86%')) and type_exam='HBA1c' and suivi_diabete.dossier_id=dossier.id and liste_exam.date_exam > '2009-01-01' AND liste_exam.date_exam < '2014-04-20' ORDER BY cabinet, dossier.id, date_consult, date_exam";
...
//      "GROUP BY nom_cab ".
//      "ORDER BY nom_cab, numero ";
$req5 = "SELECT a.cabinet, count(*), nom_cab FROM dossier, account as a WHERE infirmiere!='' and region!='' AND dossier.cabinet=a.cabinet AND actif='oui' AND (a.nom_cab like '%16%' OR a.nom_cab like '%17%' OR a.nom_cab like '%79%' OR a.nom_cab like '%86%') GROUP BY nom_cab ORDER BY nom_cab, numero";
...
//     "and suivi_diabete.dossier_id=liste_exam.id ".
//     "ORDER BY cabinet, dossier.id, date_consult, date_exam";
$req5 = "SELECT cabinet, dossier.id, date_exam, resultat1, evaluation_infirmier.date as date_consult, DATEDIFF(date_exam, evaluation_infirmier.date) as deltaj FROM dossier, liste_exam, evaluation_infirmier, suivi_diabete WHERE liste_exam.id=dossier.id AND evaluation_infirmier.id=dossier.id  and dossier.cabinet IN (SELECT cabinet FROM account WHERE (nom_cab like '%16%' OR nom_cab like '%17%' OR nom_cab like '%79%' OR nom_cab like '%86%')) and type_exam='HBA1c' and suivi_diabete.dossier_id=dossier.id and liste_exam.date_exam > '2009-01-01' AND liste_exam.date_exam < '2014-04-20' ORDER BY cabinet, dossier.id, date_consult, date_exam";
...
//      "GROUP BY nom_cab ".
//      "ORDER BY nom_cab, numero ";
$req6 = "SELECT a.cabinet, count(*), nom_cab FROM dossier, account as a WHERE infirmiere!='' and region!='' AND dossier.cabinet=a.cabinet AND actif='oui' AND (a.nom_cab like '%16%' OR a.nom_cab like '%17%' OR a.nom_cab like '%79%' OR a.nom_cab like '%86%') GROUP BY nom_cab ORDER BY nom_cab, numero";
...
//     "and suivi_diabete.dossier_id=liste_exam.id ".
//     "ORDER BY cabinet, dossier.id, date_consult, date_exam";
$req6 = "SELECT cabinet, dossier.id, date_exam, resultat1, evaluation_infirmier.date as date_consult, DATEDIFF(date_exam, evaluation_infirmier.date) as deltaj FROM dossier, liste_exam, evaluation_infirmier, suivi_diabete WHERE liste_exam.id=dossier.id AND evaluation_infirmier.id=dossier.id  and dossier.cabinet IN (SELECT cabinet FROM account WHERE (nom_cab like '%16%' OR nom_cab like '%17%' OR nom_cab like '%79%' OR nom_cab like '%86%')) and type_exam='HBA1c' and suivi_diabete.dossier_id=dossier.id and liste_exam.date_exam > '2009-01-01' AND liste_exam.date_exam < '2014-04-20' ORDER BY cabinet, dossier.id, date_consult, date_exam";
...
$req= "SELECT cabinet, dossier.id, date_exam, resultat1, ".
    "evaluation_infirmier.date as date_consult, DATEDIFF(date_exam, evaluation_infirmier.date) as deltaj ".
    "FROM dossier, liste_exam, evaluation_infirmier, suivi_diabete ".
    "WHERE liste_exam.id=dossier.id AND evaluation_infirmier.id=dossier.id and cabinet IN (SELECT cabinet FROM account WHERE (nom_cab like '%16%' OR nom_cab like '%17%' OR nom_cab like '%79%' OR nom_cab like '%86%')) and date_exam > '2009-01-01' AND date_exam < '".$date_start."'  and ".
...
$req2= "SELECT cabinet, dossier.id, date_exam, resultat1, ".
    ...
    "dossier.cabinet IN (SELECT cabinet FROM account WHERE (nom_cab like '%16%' OR nom_cab like '%17%' OR nom_cab like '%79%' OR nom_cab like '%86%')) and type_exam='LDL' ".
    "and suivi_diabete.dossier_id=liste_exam.id ".
    "ORDER BY cabinet, dossier.id, date_consult, date_exam";
$res2=mysql_query($req2) or die("erreur SQL:".mysql_error()."<br>$req");
...
$tab_csv["2"] = stripAccents(getPropertyValue("FicheCabinet:infirmiere"));
...
// Evolution tension
  ...
  for($i=1; $i<=4; $i++)
  {
    $tab_csv[$cpt_tab] = round($change["pierre"][$i]/$dossierssup140["pierre"][$i]*100).' %';
    echo '<br>##'.$change["pierre"][$i].' / '.$dossierssup140["pierre"][$i];
...
$tab_csv["81"] = $tab_cabinet['total_cogni']; 
  ...
  // entete
$tab_entete = array();
if(!file_exists('./csv/'.$current_year.'_'.$current_month.'_region.csv'))
{
  // activité
...
// Evolution tension
  ...
  $fp = fopen('./csv/'.$current_year.'_'.$current_month.'_region.csv', 'a+');
...
echo "<br>ok fin : ".$current_cabinet;//exit();
