  ...
  $total_temps = 0;
          ...
          foreach ($resultTemps as $key => $value) 
          {
            ...
            if($value['tps_reunion_infirmiere'] != null)
              $coordination += (($value['tps_reunion_infirmiere']) / 7) * $coef_diff;
...
$query = "select COUNT(DISTINCT(evaluation_infirmier.id)) as cogni_unique_patient from evaluation_infirmier,dossier where cabinet='".$current_cabinet."' and evaluation_infirmier.id = dossier.id AND type_consultation like '%cognitif%'";
...
$tab_csv["2"] = getPropertyValue("FicheCabinet:infirmiere");
$tab_csv["3"] = getPropertyValue("FicheCabinet:ville").' - '.getPropertyValue("FicheCabinet:region");
...
$tab_csv["32"] = $objExams->nb_patient;
...
$tab_csv["38"] = ($autre_type_consult < 0) ? '0' : $autre_type_consult;
...
// Evolution HBA1C
  ...
  $tab_csv["42"] = sprintf("%.2f", $prem_sup_av);;
  $tab_csv["43"] = sprintf("%.2f", $prem_sup_ap);
  $tab_csv["44"] = sprintf("%.2f", round((($prem_sup_ap/$prem_sup_av)*100)-100, 2)).' %';
  $tab_csv["45"] = sprintf("%.2f", $second_sup_av);;
  $tab_csv["46"] = sprintf("%.2f", $second_sup_ap);
  $tab_csv["47"] = sprintf("%.2f", round((($second_sup_ap/$second_sup_av)*100)-100, 2)).' %';
  $tab_csv["48"] = sprintf("%.2f", $three_sup_av);;
  $tab_csv["49"] = sprintf("%.2f", $three_sup_ap);
  $tab_csv["50"] = sprintf("%.2f", round((($three_sup_ap/$three_sup_av)*100)-100, 2)).' %';
  $tab_csv["51"] = sprintf("%.2f", $four_sup_av);;
  $tab_csv["52"] = sprintf("%.2f", $four_sup_ap);
  $tab_csv["53"] = sprintf("%.2f", round((($four_sup_ap/$four_sup_av)*100)-100, 2)).' %';
  $tab_csv["54"] = sprintf("%.2f", $five_sup_av);;
  $tab_csv["55"] = sprintf("%.2f", $five_sup_ap);
  $tab_csv["56"] = sprintf("%.2f", round((($five_sup_ap/$five_sup_av)*100)-100, 2)).' %';
  $tab_csv["57"] = sprintf("%.2f", $six_sup_av);;
  $tab_csv["58"] = sprintf("%.2f", $six_sup_ap);
  $tab_csv["59"] = sprintf("%.2f", round((($six_sup_ap/$six_sup_av)*100)-100, 2)).' %';
// fin Evolution HBA1C
...
// Evolution LDL
  ...
  $tab_csv["60"] = sprintf("%.2f", $prem_sup_av);
  $tab_csv["61"] = sprintf("%.2f", $prem_sup_ap);
  $tab_csv["62"] = sprintf("%.2f", round((($prem_sup_ap/$prem_sup_av)*100)-100, 2)).' %';
  $tab_csv["63"] = sprintf("%.2f", $second_sup_av);
  $tab_csv["64"] = sprintf("%.2f", $second_sup_ap);
  $tab_csv["65"] = sprintf("%.2f", round((($second_sup_ap/$second_sup_av)*100)-100, 2)).' %';
  $tab_csv["66"] = sprintf("%.2f", $three_sup_av);
  $tab_csv["67"] = sprintf("%.2f", $three_sup_ap);
  $tab_csv["68"] = sprintf("%.2f", round((($three_sup_ap/$three_sup_av)*100)-100, 2)).' %';
// fin Evolution LDL
...
// Evolution tension
  ...
  for($i=1; $i<=4; $i++)
  {
    $tab_csv[$cpt_tab] = ($dossierssup140[$current_cabinet][$i]==0) ? "ND" : round($change[$current_cabinet][$i]/$dossierssup140[$current_cabinet][$i]*100).' %';
    $cpt_tab++;
  }
// fin Evolution tension
...
$tab_csv["81"] = $tab_cabinet['total_cogni'];
  ...
  //$fp = fopen('./csv/test_decembre.csv', 'a+');
  $fp = fopen('./csv/'.$current_year.'_'.$current_month.'.csv', 'a+');
  fputcsv($fp, $tab_csv);
  fclose($fp);
  // fin ecriture csv
